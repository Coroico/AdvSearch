...
if ($preRecordIds === 'truncate') {
    $db->exec("TRUNCATE `{$table_prefix}advsearch_indexation`");
} else if ($preRecordIds === 'reset') {
    $db->exec("UPDATE {$table_prefix}advsearch_indexation SET is_indexed = NULL");
}
if (!empty($lstIds)) {
    foreach ($lstIds as $id) {
        $check = $db->prepare("SELECT * FROM {$table_prefix}advsearch_indexation WHERE doc_id = :id LIMIT 1");
